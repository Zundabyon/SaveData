...
- **Rails との相性**: Active Record が PostgreSQL の機能を十分にサポートしている点

### 外部サービス選定理由

**デプロイ環境（Docker + Render）**

- **環境の一貫性**: Dockerfile.dev（開発環境）と Dockerfile（本番環境）を分離することで
  ローカル・本番間の環境差異を最小化しました。
- **Render の選定理由**:
...
- Rails のデファクトスタンダードで、OAuth（Google認証）との統合が容易な点
- パスワードリセット、メール確認などの認証フローが標準実装済み
- 無料で実装できる点

**画像管理（Cloudinary）**

- 画像の最適化やリサイズ機能がユーザー体験向上に寄与しているため採用しました。
...
- アプリ機能の継続的なアップデート
- アクセス数を増加させる施策 -現在、アプリに繋がる主な動線がXがメインであり、また広告も決められた時間にのみとなっているため
  時間帯ごとのアクセス数、その他の媒体にも波及させ、まずは価値提供からフィードバックを
  月５件は頂ける仕組み、施策を続けていきます。
- チーム開発を意識する
  現在、コントローラーおよび一部モデルに債務が偏りがちなので
  ヘルパーメソッドに切り出すなどして可読性の高いコードを心がけます。
